...
const TutorialBreadcrumb = styled.div`
  display: flex;
  align-items: center;
  margin-bottom: 16px;
  font-size: 1rem;
...
const TutorialDetailPage: NextPage<Props> = ({ data }) => {
  const classes = useStyles();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { data: items, loading: isTutorialsLoading } = useArticles('TUTORIAL');
  const { data: item, loading: isTutorialLoading } = useArticle(data?.id);
  ...
  return (
    <>
      <SEO title={item?.title || 'Tutorial'} description={item?.excerpt || ''} />
          ...
          <div className={classes.topBarMobile}>
            <div className={classes.breadcrumbContainer}>
              <TutorialBreadcrumb>
                ...
                <span>›</span>
                <span>{item?.title || 'Tutorial Detail'}</span>
              </TutorialBreadcrumb>
            </div>
          </div>
        ) : (
            ...
            <MobileSidebarContainer $isOpen={sidebarOpen}>
              ...
              <StyledLeftInnerContent>
                {isTutorialsLoading ? (
                  ...
                  items.map((tutorial) => (
                    <Link href={`/tutorials/${tutorial.id}`} passHref key={tutorial.id}>
                      <ArticleLink 
                        className={router.query.id === tutorial.id ? 'active' : ''}
                      >
                        <span style={{ color: secondaryColor }}>{tutorial.title}</span>
                        {router.query.id === tutorial.id && (
                          <Chip 
                            label="Current" 
                            size="small" 
                            color="primary" 
            ...
            {isTutorialLoading ? (
              ...
              <>
                <div className={classes.actionBar}>
                  <Typography variant="h3" component="h1" className={classes.tutorialTitle}>
                    {item.title}
                  </Typography>
              ...
              <Box textAlign="center" py={10}>
                <Typography variant="h5" color="textSecondary">
                  Tutorial not found. Please try another tutorial.
                </Typography>
                <Button 
                  variant="contained" 
                  color="primary" 
